...
export default function UserDetailPage({ params }: PageProps) {
  ...
  const sessionId = params.slug[1] || null
  const workflowId = params.slug[2] || null
  
  // For deep linking to specific session/workflow
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Find the user specified in the URL
      const targetUser = data.find(user => user.user_id === userId)
      
      if (targetUser) {
        setSelectedUser(targetUser)
        
        if (sessionId && workflowId) {
          console.log('Deep linking to session:', { sessionId, workflowId })
        }
      } else {
        // User not found, fallback to first user or show error
        if (data.length > 0) {
  ...
  const handleUserSelect = (user: UserData) => {
    ...
    // Update URL to reflect selected user (without session/workflow)
    window.history.pushState({}, '', `/users/${user.user_id}`)
  }
